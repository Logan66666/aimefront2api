...
function recordRequest(model, stream, success, tokens, durationMs) {
  ...
  // 按模型维度响应时间（每模型保留最近 50 次）
  if (!stats.modelUsage[model].responseTimes) stats.modelUsage[model].responseTimes = [];
  stats.modelUsage[model].responseTimes.push(durationMs);
  if (stats.modelUsage[model].responseTimes.length > 50) stats.modelUsage[model].responseTimes.shift();

  // 按模型维度时间线（用于模型卡片的时间线图）
  if (!stats.modelUsage[model].timeline) stats.modelUsage[model].timeline = {};
  const minKey = new Date().toISOString().slice(0, 16).replace("T", " ");
  stats.modelUsage[model].timeline[minKey] = (stats.modelUsage[model].timeline[minKey] || 0) + 1;
  const mKeys = Object.keys(stats.modelUsage[model].timeline);
  if (mKeys.length > 120) { for (const k of mKeys.slice(0, mKeys.length - 120)) delete stats.modelUsage[model].timeline[k]; }
...
app.get("/dashboard", (req, res) => {
  ...
  // ── 模型卡片数据：基于 MODEL_MAP 主模型（非别名）生成 ──
  const PRIMARY_MODELS = ["deepseek-v4-pro", "deepseek-v4-flash", "glm-5.1"];
  const providerLabel = { deepseek: "DeepSeek", glm: "智谱 GLM" };
  const modelCards = PRIMARY_MODELS.map(mid => {
    const meta = MODEL_MAP[mid] || {};
    const d = stats.modelUsage[mid] || { requests: 0, tokensIn: 0, tokensOut: 0, errors: 0, responseTimes: [], timeline: {} };
    const total = d.requests;
    const okCount = total - d.errors;
    const availPct = total > 0 ? ((okCount / total) * 100).toFixed(2) : "—";
    const availColor = total === 0 ? "var(--muted)" : okCount / total >= 0.95 ? "var(--green)" : okCount / total >= 0.7 ? "var(--amber)" : "var(--red)";
    const statusTag = total === 0 ? '<span class="mc-status mc-idle">空闲</span>'
      : okCount / total >= 0.95 ? '<span class="mc-status mc-ok">正常</span>'
      : '<span class="mc-status mc-err">错误</span>';
    const avgLatency = d.responseTimes && d.responseTimes.length > 0
      ? Math.round(d.responseTimes.reduce((a, b) => a + b, 0) / d.responseTimes.length) + ' ms'
      : '— ms';
    // 按模型时间线图
    const tlEntries = Object.entries(d.timeline || {}).sort().slice(-60);
    const tlMax = Math.max(1, ...tlEntries.map(e => e[1]));
    const tlBars = tlEntries.map(([time, count]) => {
      const hasErr = count > 0; // 简化：有记录即视为成功点
      return '<div class="mc-tl-bar' + (hasErr ? '' : ' mc-tl-empty') + '" title="' + time + ': ' + count + '"><div class="mc-tl-fill" style="height:' + (tlMax > 0 ? Math.round(count / tlMax * 100) : 0) + '%"></div></div>';
    }).join('');
    return '<div class="mc-card">'
      + '<div class="mc-header"><span class="mc-icon">⚙</span><span class="mc-name">' + mid + '</span>' + statusTag + '</div>'
      + '<div class="mc-sub"><span class="mc-provider">' + (providerLabel[meta.providerID] || meta.providerID) + '</span><span class="mc-mid mono">' + mid + '</span></div>'
      + '<div class="mc-metrics"><div class="mc-metric"><div class="mc-metric-label">⟡ 对话延迟</div><div class="mc-metric-val">' + avgLatency + '</div></div><div class="mc-metric"><div class="mc-metric-label">⊕ 端点 PING</div><div class="mc-metric-val">— ms</div></div></div>'
      + '<div class="mc-avail-row"><span class="mc-avail-label">可用性 · ' + (total > 0 ? Math.min(total, 9999) + ' 次' : '暂无') + '</span><span class="mc-avail-pct" style="color:' + availColor + '">' + availPct + '%</span></div>'
      + '<div class="mc-tl-section"><span class="mc-tl-label">近 60 次记录</span><span class="mc-tl-count">' + total + ' 总刷新</span><div class="mc-tl-chart">' + (tlBars || '<div class="mc-tl-empty-text">暂无数据</div>') + '</div><div class="mc-tl-axis"><span>PAST</span><span>NOW</span></div></div>'
      + '</div>';
  }).join('');

  // ── 模型表格：显示全部主模型（不再只显示有数据的）──
  const modelTableRows = PRIMARY_MODELS.map(mid => {
    const d = stats.modelUsage[mid] || { requests: 0, tokensIn: 0, tokensOut: 0, errors: 0 };
    return '<tr><td class="mono">' + mid + '</td><td class="num">' + d.requests + '</td><td class="num">' + formatTokens(d.tokensIn) + '</td><td class="num">' + formatTokens(d.tokensOut) + '</td><td class="num" style="color:' + (d.errors > 0 ? 'var(--red)' : 'var(--muted)') + '">' + d.errors + '</td><td><div style="display:flex;align-items:center;gap:6px"><div class="bar-wrap" style="width:80px"><div class="bar-fill" style="width:' + (stats.totalRequests > 0 ? (d.requests / stats.totalRequests * 100).toFixed(0) : 0) + '%"></div></div>' + (stats.totalRequests > 0 ? (d.requests / stats.totalRequests * 100).toFixed(0) : 0) + '%</div></td></tr>';
  }).join('');
